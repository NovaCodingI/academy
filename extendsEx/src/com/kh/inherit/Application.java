...
public class Application {
	...
	public static void main(String[] args) {
		...
		/* 1. 최대 10명의 사원 정보를 기록할 수 있게 객체 배열을 생성
		 * 2. 키보드로 사원 정보를 입력 받도록 구현*/
		// employee타입을 담을수있는 10개의 방이 생성되고
		// 각 방은 참조변수의 초기값인 null로 초기화 된다.
		...
		while(true) {
			...
			String name = scan.next(); // 변수에 담아주어야 재사용 가능
			...
			// 배열에 저장
			empArry[index] = emp;
			// 똑같은 방에 넣을순 없으니 "사원이 입력된 후 다음번 방을 가르킴"
			index++;
		...
		for(Employee e : empArry) {
			// null 체크
			// 배열이 초기화 될때 null 값을 가지고 있으므로
			// e. <-- 주소접근연산자인 .을 이용할 경우
			// null 은 주소를 가지고 있지 않으므로 오류가 발생!!
			
//			Exception in thread "main" java.lang.NullPointerException: Cannot invoke "com.kh.inherit.practice.Employee.infomation()" because "e" is null
//			at extendsEx/com.kh.inherit.Application.main(Application.java:76)
